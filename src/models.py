...
from src.config import (
    PANEL_WIDTH, PANEL_HEIGHT, GAP_SIZE, QUADRANT_WIDTH, QUADRANT_HEIGHT,
    INTER_UNIT_GAP  # <--- IMPORTED THIS
)
...
class BuildUpLayer:
    ...
    def _add_plotting_coordinates(self):
        ...
        # --- FIX START: Correct Unit Calculation (Subtracting Gaps) ---
        # We must subtract the total gap space before dividing by count
        total_gap_w = (self.panel_cols - 1) * INTER_UNIT_GAP
        total_gap_h = (self.panel_rows - 1) * INTER_UNIT_GAP

        cell_width = (quad_width - total_gap_w) / self.panel_cols
        cell_height = (quad_height - total_gap_h) / self.panel_rows
        # --- FIX END ---
        ...
        # --- FIX START: Add Gap to Base Position ---
        # Position = Index * (Unit_Size + Gap)
        plot_x_base_raw = local_index_x_raw * (cell_width + INTER_UNIT_GAP)
        plot_y_base = local_index_y * (cell_height + INTER_UNIT_GAP)
        # --- FIX END ---
        ...
        # FIX: Add Gap here too
        plot_x_base_flipped = local_index_x_flipped * (cell_width + INTER_UNIT_GAP)
        ...
        # FIX: Add Gap here too
        plot_x_base_raw_phys = local_index_x_raw_phys * (cell_width + INTER_UNIT_GAP)
        
        x_offset_raw_phys = np.where(df['PHYSICAL_X_RAW'] >= self.panel_cols, quad_width + self.gap_x, 0)

        if use_spatial_coords:
            # Safe Bet: Map Flipped X to grid center for robustness in Stack View
